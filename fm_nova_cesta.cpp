...
fm_nova_cesta::fm_nova_cesta(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::fm_nova_cesta)
{
    ui->setupUi(this);

    fm_nova_cesta::inicializa_fm_nova_cesta();
}

fm_nova_cesta::~fm_nova_cesta()
{
    delete ui;
}
...
void fm_nova_cesta::inicializa_fm_nova_cesta()
{
    fm_nova_cesta::atualizaTableWidgetProdutos();

    fm_nova_cesta::cabecalho = {"qtd", "Nome", "Venda", "Sub-Total", "id_produto", "Descricao", "Custo", "id"};
    global_functions::formatTableWidget(ui->tableWidget_itens_cesta, cabecalho);
    ui->tableWidget_itens_cesta->setColumnHidden(7, true);
}

void fm_nova_cesta::atualizaTableWidgetProdutos()
{
    QVector<Produto> listaProdutos;
    ProdutoController produto_controller;
    listaProdutos = produto_controller.buscaTodos();

    fm_nova_cesta::populaTableWidgetProdutos(listaProdutos);
}

void fm_nova_cesta::populaTableWidgetProdutos(QVector<Produto> listaProdutos)
{
    Produto produto;
    fm_nova_cesta::cabecalho = {"Nome", "Valor", "id", "Descrição", "Custo"};
...
void fm_nova_cesta::removeProduto()
{
    int row, quantidade;
    ...
    item_cesta.setCesta(fm_nova_cesta::cesta);
    item_cesta.setProduto(produto);

    fm_nova_cesta::adicionaItemCesta(item_cesta);

    ui->tableWidget_produtos->removeRow(row);
}

void fm_nova_cesta::adicionaItemCesta(ItemCesta item_cesta)
{
    int row;
    double subtotal;
...
void fm_nova_cesta::removeItemCesta()
{
    ...
    fm_nova_cesta::adicionaProduto(produto);
}

void fm_nova_cesta::adicionaProduto(Produto produto)
{
    int row;
...
void fm_nova_cesta::limpaFormulario()
{
        ui->lineEdit_nome->clear();
        ui->lineEdit_descricao->clear();
        ui->lineEdit_custo->clear();
        ui->lineEdit_valor->clear();
        ui->tableWidget_itens_cesta->clear();
        ui->tableWidget_itens_cesta->setRowCount(0);
        ui->label_total->setText("R$ 0.00");

        fm_nova_cesta::atualizaTableWidgetProdutos();

        fm_nova_cesta::cabecalho = {"qtd", "Nome", "Venda", "Sub-Total", "id_produto", "Descricao", "Custo"};
        global_functions::formatTableWidget(ui->tableWidget_itens_cesta, cabecalho);
...
void fm_nova_cesta::on_pushButton_fechar_clicked()
{
    close();
}

void fm_nova_cesta::on_pushButton_adicionar_clicked()
{
    int row;
    row = ui->tableWidget_produtos->currentRow();

    if (row >= 0)
        fm_nova_cesta::removeProduto();
}

void fm_nova_cesta::on_tableWidget_produtos_itemSelectionChanged()
{
    ui->pushButton_adicionar->setEnabled(true);
}

void fm_nova_cesta::on_tableWidget_itens_cesta_itemSelectionChanged()
{
    ui->pushButton_retirar->setEnabled(true);
}

void fm_nova_cesta::on_pushButton_retirar_clicked()
{
    int row;
    row = ui->tableWidget_itens_cesta->currentRow();

    if (row >= 0)
        fm_nova_cesta::removeItemCesta();
}

void fm_nova_cesta::on_pushButton_salvar_cesta_clicked()
{
    ...
    fm_nova_cesta::cesta.setNome(nome);
    fm_nova_cesta::cesta.setDescricao(descricao);
    fm_nova_cesta::cesta.setCusto(custo);
    fm_nova_cesta::cesta.setValor(valor);

    fm_nova_cesta::cesta = cesta_controller.insere(fm_nova_cesta::cesta);

    ui->lineEdit_id->setText(QString::number(fm_nova_cesta::cesta.getId()));

    QMessageBox::information(this, "", "Cesta gravada com sucesso!\n\nFavor inserir os itens!");
    ui->pushButton_salvar_cesta->setEnabled(false);
    ui->tableWidget_produtos->setEnabled(true);
    ui->tableWidget_itens_cesta->setEnabled(true);
    ui->pushButton_adicionar->setEnabled(true);
    ui->pushButton_retirar->setEnabled(true);
    ui->spinBox_quantidade->setEnabled(true);
}
